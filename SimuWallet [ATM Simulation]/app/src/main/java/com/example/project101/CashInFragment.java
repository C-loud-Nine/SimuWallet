...
public class CashInFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment_cash_in, container, false);

        transactionListView = rootView.findViewById(R.id.transactionListViewin);
        ...
        // Retrieve the user's Cash In transactions from Firebase
        DatabaseReference cashInRef = FirebaseDatabase.getInstance().getReference("users/" + cardno + "/transactionHistory");
        // Modify the query based on your data structure, e.g., orderByChild("type").equalTo("Credited")

        cashInRef.addValueEventListener(new ValueEventListener() {
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    for (DataSnapshot transactionSnapshot : dataSnapshot.getChildren()) {
                        ...
                        // Check if the transaction is a Cash In (e.g., Credited)
                        if ("Credited".equals(type) || "Received".equals(type)) {
                            TransactionHistoryItem item = new TransactionHistoryItem(amount, date, type);
                            transactionHistory.add(item);
                        }
                    }

                    // Set up the adapter to display the Cash In transaction history
